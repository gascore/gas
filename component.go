...
func UpdateComponent(_parent dom.Element, new interface{}, old interface{}, index int) error {
	...
	_childes := _parent.ChildNodes() // get _parent childes elements
	if _childes == nil {
	...
	isChanged, err := changed(new, old); if err != nil {
		return err
	}
	if isChanged {
		_new, err := CreateComponent(new)
	...
	// Check if component childes has changed
	// new and old is components
	if isComponent(new) && isComponent(old) {
